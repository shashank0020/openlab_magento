...
class Product(osv.Model):
    ...
    def update_from_magento(
        ...
        website_obj = self.pool.get('magento.instance.website')
        magento_product_obj = self.pool.get('magento.website.product')
        magento_cat_obj=self.pool.get('magento.instance.product_category')
        ...
        with magento.Product(
            ...
            try:
                ...
                if e:
                    ...
                    #Make connection
                    url,username,password=website.instance.url+'/index.php/api/v2_soap?wsdl=1',website.instance.api_user,website.instance.api_key
                    wsdl_file =url
                    ...
                    magento_cat_id_search=magento_cat_obj.search(cursor,user,[('category','=',product.product_tmpl_id.categ_id.id)])
                    magento_cat_id=magento_cat_obj.browse(cursor,user,magento_cat_id_search[0]).magento_id
                    
                    
                    #Gather product information
                    product_details = {'name':product.name,'description':product.description, 'short_description':product.description,'weight':'0.00', 'status':'1','url_key':None,'url_path':None,'visibility' :4,'price':product.product_tmpl_id.list_price,'tax_class_id':1,'categories': [magento_cat_id],'websites': [website.id]}
                    attributeSets = client.service.catalogProductAttributeSetList(session)
                    #create missing product in magento
                    
                    missing_prod_id=client.service.catalogProductCreate(session , 'simple', attributeSets[0].set_id,product.default_code+'(OpenERP)',product_details)
                    
                    #update magento id of existing product in OpenERP
                    cursor.execute('update magento_website_product set magento_id=%s where id=%s',(missing_prod_id,magento_product_id))
                    
                    product_data={'set': '4', 'msrp_enabled': None, 'weight': '0.00', 'type_id': 'simple', 'options_container': '', 'small_image_label': None, 'thumbnail_label': None, 'special_from_date': None, 'url_key': None, 'sku': product.default_code+'(OpenERP)', 'country_of_manufacture': None, 'custom_layout_update': None, 'msrp_display_actual_price_type': None, 'page_layout': None, 'minimal_price': None, 'category_ids': [str(magento_cat_id)], 'meta_keyword': None, 'gift_message_available': None, 'custom_design_to': None, 'short_description': product.description, 'meta_description': None, 'type': 'simple', 'tax_class_id': '1', 'status': '1', 'group_price': [], 'meta_title': None, 'description': product.description, 'news_to_date': None, 'old_id': None, 'price': product.product_tmpl_id.list_price, 'news_from_date': None, 'visibility': '4', 'updated_at': None, 'tier_price': [], 'special_price': None, 'recurring_profile': None, 'is_recurring': None, 'url_path': None, 'categories': [str(magento_cat_id)], 'name': product.name, 'custom_design_from': None, 'required_options': '0', 'product_id':str(missing_prod_id) , 'created_at': None, 'websites': [str(website.id)], 'special_to_date': None, 'custom_design': None, 'has_options': '0', 'msrp': None, 'image_label': None}
                else:
                    pass
